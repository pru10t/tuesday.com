...
export const TaskCard: React.FC<TaskCardProps> = ({ task, isSimulated = false, isDelayedSource = false, isOverbooked = false, onDragStart }) => {
  ...
  const borderClass = isDelayedSource
    ? 'ring-2 ring-neutral-900 ring-offset-1 border-transparent'
    : isOverbooked
  ...
  return (
    <div
      ...
      className={`bg-white p-3.5 rounded-xl border hover:shadow-sm transition-all duration-200 mb-3 group relative cursor-grab active:cursor-grabbing ${borderClass} ${isSimulated ? 'opacity-95' : ''}`}
    >
      <div className="flex justify-between items-start gap-2">
        <div className="flex items-start gap-2 overflow-hidden">
          ...
          />
          <h4 className="font-medium text-neutral-700 text-sm leading-snug truncate">{task.title}</h4>
        </div>
      ...
      <div className="flex items-center justify-between mt-3 pl-4">
        ...
        {isDelayedSource && (
          <span className="text-neutral-900 font-semibold flex items-center text-[10px] bg-neutral-100 px-1.5 py-0.5 rounded">
      ...
      {/* Cost and Effort */}
      <div className="flex items-center gap-3 mt-2 pl-4 text-[10px] text-neutral-400">
      ...
      <div className="flex items-center justify-between mt-2 pt-2 border-t border-neutral-100 pl-4">
        <div className="flex items-center text-neutral-400 text-xs font-medium">
          <Calendar size={12} className="mr-1.5 text-neutral-300" />
